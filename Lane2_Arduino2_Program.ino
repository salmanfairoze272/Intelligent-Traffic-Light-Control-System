...
const int Lane2_Sensor1 = A0;               // output of IR Sensor 1 is fed to analog input 0 
const int Lane2_Sensor2 = A1;               // output of IR Sensor 2 is fed to analog input 1
...
int T2;                                   // float variable to pass the T1, which is an input of the NN, to main Mega board 
int Incoming_Value = 0;                     // to read the signal sent by the main Mega board

void setup() {
  Serial.begin(9600);

  Sensor1_Value = analogRead(Lane2_Sensor1);
  if(Sensor1_Value < 200){
    Sensor1_Time = millis();
    while(analogRead(Lane2_Sensor2) > 200);
    Sensor2_Value = analogRead(Lane2_Sensor2);
    if(Sensor2_Value < 200){
      Sensor2_Time = millis();
      T2 = (Sensor2_Value - Sensor1_Value)/1000;
      Serial.write(T2);
    }
  }
}
