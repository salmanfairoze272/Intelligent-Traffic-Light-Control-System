...
float n1_last_weights[3];
float n2_last_weights[3];
float n3_last_weights[3];
...
int x, y = 0;                                   // integer variables to get recieved ASCII serial inputs
int c, d = 0;                                // integer variables to convert ASCII from serial 1 into integers and to calculate ultimate results 
int e, f = 0;                                // integer variables to convert ASCII from serial 2 into integers and to calculate ultimate results
int g, h = 0;                                // integer variables to convert ASCII from serial 3 into integers and to calculate ultimate results

float summation(int a,float new_weights[3]);    // fn to find sum of weighed inputs
void lane_1(float time1);                      // To pass the green light signals to the LEDs (Outputs)
void lane_2(float time2);                      // To pass the green light signals to the LEDs (Outputs)
void lane_3(float time3);                      // To pass the green light signals to the LEDs (Outputs)
...
void setup() {
  ...
  delay(100);
  Serial1.begin(9600);                          // set the baude rate for serial communication
  delay(100);
  Serial2.begin(9600);                          // set the baude rate for serial communication
  delay(100);
  Serial3.begin(9600);                          // set the baude rate for serial communication
  delay(100);
...
/* to find changes in weights and final weights, for each input of three inputs in an input set */
  ...
  for(int i=0; i<3; i++){
    n1_last_weights[i] = n1_ultimate_weights[i];
  }
...
/* calculating output, changes in weights and final weightsfor the SECOND NEURON */   
  ...
  for(int i=0; i<3; i++){
    n2_last_weights[i] = n2_ultimate_weights[i];
  }
...
/* calculating output, changes in weights and final weightsfor the THIRD NEURON */    
  ...
  for(int i=0; i<3; i++){
    n3_last_weights[i] = n3_ultimate_weights[i];
  }
...
/* Serial communication 3 */  
  ...
  T3 = (g*10)+h;  // Time gap of lane 3 of sensor one and sensor 2
}
...
void loop() {
  ...
  /* to calculate the output value for neuron 1 */
    sum2 = 0;
    for(int i=0; i<3; i++){
      sum2 += new_inputs[i]*n1_last_weights[i];
    }
    if(sum2 <= 5){
      Output1 = 60;
    }
  ...
  /* to calculate the output value for neuron 2 */
    sum2 = 0;
    for(int i=0; i<3; i++){
      sum2 += new_inputs[i]*n2_last_weights[i];
    }
    if(sum2 <= 5){
      Output2 = 60;
    }
  ...
  /* to calculate the output value for neuron 3 */
    sum2 = 0;
    for(int i=0; i<3; i++){
      sum2 += new_inputs[i]*n3_last_weights[i];
    }
    if(sum2 <= 5){
      Output3 = 60;
    }
    ...
    Serial.println();
    Serial.println("Output 3 is: ");
    Serial.print(Output3);
    delay(100);
    Serial.println();

/* convert the output times in miliseconds, into seconds by multiplying by 1000 */
...
/* Serial communication 1 with Arduino Uno 1 */  
  x = 0;
  //while(Serial1.available() == 0);
  Serial1.write(6);
  while(Serial1.available() == 0);
...
/* Serial communication 2 with Arduino Uno 2 */
  x = 0;
  //while(Serial2.available() == 0);
  Serial2.write(6);
  while(Serial2.available() == 0);
...
/* Serial communication 3 with Arduino Uno 3*/  
  delay(100);
  x = 0;
  //while(Serial3.available() == 0);
  Serial3.write(6);
