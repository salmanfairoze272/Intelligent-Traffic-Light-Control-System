/* analog input pins to read IR sensors */
const int L1_S1 = A0; 
const int L1_S2 = A1;
const int L2_S1 = A2;
const int L2_S2 = A3;
...
float new_inputs[3];                          // to have sensor inputs in the calculation
                                                                            
int a;                                          // to pass the row number (number of input set) of inputs matrix
int b;                                          // to pass the column number of each array                            
float sum2 = 0;
unsigned long Sensor1_Time, Sensor2_Time;       // variables to get the time when each sensor detects a vehicle
float T1, T2, T3 = 0;                           // variables to get time differences between each sensor
int Sensor1_Value, Sensor2_Value = 0;           // to read sensor values from analog input pins
...
void loop() {
  Sensor1_Value = analogRead(L1_S1);    // read 
  if(Sensor1_Value < 200){
    Sensor1_Time = millis();
    Sensor2_Value = analogRead(L1_S2);
    if(Sensor2_Value < 200){
      Sensor2_Time = millis();
      T1 = (Sensor2_Time - Sensor1_Time)/1000;
      Sensor1_Value = 0;
      Sensor2_Value = 0;
      Sensor1_Time = 0;
      Sensor2_Time = 0;
  ...
  Sensor1_Value = analogRead(L2_S1);
  if(Sensor1_Value < 200){
    Sensor1_Time = millis();
    Sensor2_Value = analogRead(L2_S2);
    if(Sensor2_Value < 200){
      Sensor2_Time = millis();
      T2 = (Sensor2_Time - Sensor1_Time)/1000;
      Sensor1_Value = 0;
      Sensor2_Value = 0;
      Sensor1_Time = 0;
      Sensor2_Time = 0;    
  ...
  Sensor1_Value = analogRead(L3_S1);
  if(Sensor1_Value < 200){
    Sensor1_Time = millis();
    Sensor2_Value = analogRead(L3_S2);
    if(Sensor2_Value < 200){
      Sensor2_Time = millis();
      T3 = (Sensor2_Time - Sensor1_Time)/1000;
      Sensor1_Value = 0;
      Sensor2_Value = 0;
      Sensor1_Time = 0;
      Sensor2_Time = 0;    
  ...
  float new_array[3] = {T1, T2, T3};
  for(int i=0; i<3; i++){
    new_inputs[i] = new_array[i];
  }
